...
/* I = Inativado                                                   /
/******************************************************************/
/* Valores para CONT_IN_TIPO                                       /
/* FC = Contato realizado pela seção Fale Conosco do Site          /
/* LP = Contato realizado pela Pagina de Captura                   /
/******************************************************************/
CREATE TABLE `CONTATO` (
 ...
 `CONT_IN_ORIGEM` VARCHAR(2) NOT NULL DEFAULT 'FC' COMMENT 'Fonte da origem do contato realizado',
...
(
    ...
    `CAMP_NU_LATITUDE` DECIMAL(10,8) NOT NULL DEFAULT '0.0'  COMMENT 'latitude';
    `CAMP_NU_LONGITUDE` DECIMAL(11,8) NOT NULL DEFAULT '0.0'  COMMENT 'longitude';
...
(
    `CAQR_ID` VARCHAR(100) NOT NULL COMMENT 'ID da qrcode',
    `CAQR_ID_PARENT` VARCHAR(100) COMMENT 'ID da qrcode parent',
    `CAMP_ID` int(11) NOT NULL COMMENT 'ID da campanha',
    `CAQR_TX_QRCODE`  VARCHAR(100)  NOT NULL  COMMENT 'qrcode' ,
    `CAQR_TX_QRCODEP`  VARCHAR(100)  NOT NULL  COMMENT 'qrcode impressão' ,
    `CAQR_NU_ORDER`  int(10)  NOT NULL DEFAULT 0 COMMENT 'Ordenamento',
    `CAQR_TX_TICKET`  VARCHAR(100)  NOT NULL  COMMENT 'Ticket Reduzido' ,
    `USUA_ID_GERADOR`  int(11) NOT NULL DEFAULT 1 COMMENT 'ID do usuário gerador',
    `CAQR_IN_STATUS` varchar(1)  NOT NULL DEFAULT 'A' COMMENT 'Status',
    `CAQR_DT_CADASTRO` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Data de Cadastro',
    `CAQR_DT_UPDATE` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'Data de Atualização',
...
ALTER TABLE USUARIO_TIPO_EMPREENDIMENTO
    ...
    FOREIGN KEY (TIEM_ID)
    REFERENCES TIPO_EMPREENDIMENTO(TIEM_ID) ON DELETE CASCADE;
