...
/* DML PARA LANÇAR DEBITO NO CASHBACK CC QUANDO DER ALGUM CREDITO EXCESSIVO JUNTA10 */
INSERT INTO `CAMPANHA_CASHBACK_CC` (`USUA_ID`, `USUA_ID_DONO`, `CACC_TX_DESCRICAO`, `CACC_VL_RECOMPENSA`, `CACC_IN_TIPO`) VALUES
(:USUA_ID, 1, 'Estorno de Bonificação Junta10 apoio instagram - autenticação => d3740633d8a363b72adcaf05aa837f40012cd6ae', -1.97, 'D');


/*
	QUERY PARA RETONAR SOMENTE NUMEROS DE SORTEIOS PARA CLIENTES QUE PARTICIPAM DE CAMPANHAS OU
    INDICARAM A PLATAFORMA DESDE QUE O PLANO DO DONO DA CAMPANHA SEJA PAGO
*/
...
, B.`CART_ID`
, F.`UCST_NU_TICKET`
FROM PLANO_USUARIO D
INNER JOIN `CAMPANHA` A ON A.USUA_ID = D.USUA_ID
INNER JOIN CARTAO B ON B.`CAMP_ID` = A.CAMP_ID
INNER JOIN USUARIO C ON C.USUA_ID = B.USUA_ID
INNER JOIN `USUARIO_CAMPANHA_SORTEIO` E ON E.USUA_ID = C.USUA_ID
INNER JOIN `USUARIO_CAMPANHA_SORTEIO_TICKETS` F ON F.USCS_ID = E.`USCS_ID`
WHERE D.`PLUS_ID` = (SELECT MAX(PLUS_ID) FROM PLANO_USUARIO E WHERE E.USUA_ID = D.`USUA_ID` AND E.PLUS_IN_STATUS = 'A' AND E.PLAN_ID <> 2  )
AND B.CART_IN_STATUS IN ('A','0','1','2','3','4')
AND E.`USCS_IN_STATUS` = 'A';

/* DESCOBIR CREDITOS EXPIRADOS ACIMA DE 90 DIAS PARA REALIZAR LANÇAMENTO DE ESTORNO */
select *
from CAMPANHA_CASHBACK_CC
where CACC_ID not in (
                       select CACC_ID_PARENT
                       from CAMPANHA_CASHBACK_CC
                       where CACC_ID_PARENT is not null
                      )
and DATEDIFF(CURDATE(), DATE(CACC_DT_CADASTRO)) > 90 
AND CACC_IN_TIPO = 'C'


/* REALIZAR PAR DO LANÇAMENTO DE ESTORNO CREDITOS EXPIRADOS ACIMA DE 90 DIAS */
insert into CAMPANHA_CASHBACK_CC
(USUA_ID,USUA_ID_DONO,CACC_TX_DESCRICAO, CACC_VL_RECOMPENSA,CACC_ID_PARENT,CACC_IN_TIPO)
VALUES (
:USUA_ID,
1,
'EXPIRAÇÃO CREDITO 90 DIAS CONFORME TERMO DE USO'
,:VALOR_ESTORNO
,:CACC_ID
, 'D'
);